...
# ---------------------------------------------------------
# 3. 데이터 로드 및 정제 함수 (다국어 보존 및 에러 방지)
# ---------------------------------------------------------
def safe_text(text):
...
def load_data():
    try:
        ...
        df['track_name'] = df['track_name'].apply(fix_text).apply(safe_text)
        df['artist(s)_name'] = df['artist(s)_name'].apply(fix_text).apply(safe_text)
...
# 5. 메인 화면 및 알고리즘 실행
# ---------------------------------------------------------
st.title("🎧 Weather Playlist v7.5")
st.write("실시간 기상 상황과 사용자의 감성을 분석하여 최적의 사운드 트랙을 제안합니다.")
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                        ...
                        <div style="flex: 1; text-align: right;">
                            ...
                            <p style="color: #b3b3b3; font-size: 11px; margin-top: 15px;">
                                BPM: {row['bpm']} | Release: {row['released_month']}월
                            </p>
                        </div>
                    </div>
                </div>
